...
estimate_gas() {
	...
	TXPRICE_RESPONSE=$(curl -s https://api.txprice.com)
	response=$(jq '.code' <<<"$TXPRICE_RESPONSE")
	if [[ $response != "200" ]]; then
		echo "Could not get gas information from ${TPUT_BOLD}txprice.com${TPUT_RESET}: https://www.txprice.com"
		...
		echo "Gas prices from ${TPUT_BOLD}txprice.com${TPUT_RESET}: https://www.txprice.com"
		echo " \
